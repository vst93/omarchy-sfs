...
If the `sfs` binary is not installed, the panel offers **Install SFS…** — after your confirmation it runs SFS's official install script in a terminal. Omarchy plugins have no install hooks, so this is the one-click path.
...
未安装 `sfs` 时，面板提供 **安装 SFS…**：确认后在终端运行 SFS 官方安装脚本（Omarchy 插件没有安装钩子，这是一键路径）。
