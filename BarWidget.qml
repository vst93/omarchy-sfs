...
BarWidget {
    ...
    readonly property color iconColor: {
        if (root.ready && root.hasUrgent)
            return Color.urgent;
        if (!root.ready)
            return Qt.darker(root.bar ? root.bar.barForeground : Color.foreground, 1.5);
    ...
    function t(key) {
        return tr[lang] && tr[lang][key] ? tr[lang][key] : tr["en"][key];
    }
    ...
    function spawn(binPath) {
        phase = "starting";
        webProc.command = ["/bin/sh", "-c", "exec env -u DISPLAY -u WAYLAND_DISPLAY " + Lib.shellQuote(binPath) + " web " + reqPort];
    ...
    function connectTo(port) {
        ...
        settingsGet(function (payload, text) {
            if (payload && payload.settings)
                root.autoSyncEnabled = payload.settings.autoSync === true;
        });
    }
    ...
    function syncAll(isAuto, cb) {
        ...
        syncing = true;
        apiReq("POST", "/api/sync", JSON.stringify({
            syncType: "",
            isAuto: !!isAuto
    ...
    function syncSingle(id, syncType, cb) {
        ...
        syncing = true;
        apiReq("POST", "/api/sync/single", JSON.stringify({
            id: id,
            syncType: syncType
        }), function (payload, text) {
            root.syncing = false;
    ...
    function settingsGet(cb) {
        apiReq("GET", "/api/settings", "", cb);
    }
    function settingsSave(data, cb) {
        apiReq("POST", "/api/settings", JSON.stringify(data), function (payload, text) {
            if (payload && payload.message)
                root.settingsGet(function (s) {
                    if (s && s.settings)
                        root.autoSyncEnabled = s.settings.autoSync === true;
                });
            if (cb)
                cb(payload, text);
        });
    }
    function settingsTest(w, cb) {
        apiReq("POST", "/api/settings/test", JSON.stringify(w), cb);
    }
    function addFile(path, note, cb) {
        apiReq("POST", "/api/files/add", JSON.stringify({
            filePath: path,
            note: note || ""
        }), cb);
    }
    function deleteFile(id, cb) {
        apiReq("POST", "/api/files/delete", JSON.stringify({
            id: id
        }), function (payload, text) {
            root.refresh();
            if (cb)
    ...
    function setFileDir(id, dir, cb) {
        apiReq("POST", "/api/files/dir", JSON.stringify({
            id: id,
            dir: dir || ""
        }), function (payload, text) {
            root.refresh();
    ...
    function setNote(id, note, cb) {
        apiReq("POST", "/api/files/note", JSON.stringify({
            id: id,
            note: note || ""
        }), function (payload, text) {
            root.refresh();
    ...
    function setLang(l) {
        ...
        writeProc.command = ["/bin/sh", "-c", "mkdir -p " + Lib.shellQuote(root.stateDir + "/omarchy-sfs") + " && printf '%s' " + Lib.shellQuote(l) + " > " + Lib.shellQuote(root.stateFile.replace(/endpoint\.json$/, "lang.txt"))];
        writeProc.running = true;
    }

    function openWebUI() {
        if (endpoint !== "" && root.bar)
            root.bar.run("xdg-open " + root.endpoint);
    }

    // Full teardown + fresh locate. Used by the panel's retry button and the
    // right-click refresh when the backend is gone.
    function failQueuedRequests(reason) {
        var queued = apiQueue.splice(0, apiQueue.length);
        for (var i = 0; i < queued.length; i++) {
            if (queued[i].cb)
                queued[i].cb(null, reason || "offline");
        }
    }
    function relocate() {
        endpoint = "";
        failQueuedRequests("offline");
        model = null;
        apiFails = 0;
        spawnFails = 0;
        locate();
    ...
    function pumpApi() {
        if (root.apiBusy || root.apiQueue.length === 0 || root.endpoint === "")
            return;
        root.apiBusy = true;
        ...
        if (job.method === "POST")
            args.push("-X", "POST", "-H", "Content-Type: application/json", "-d", job.body || "{}");
    ...
    Process {
        ...
        onExited: function (exitCode) {
            ...
            root.endpoint = "";
            root.failQueuedRequests("offline");
            root.model = null;
            root.phase = "locating";
            root.locate();
        }
    ...
    Timer {
        ...
        repeat: true
        onTriggered: {
            if (root.autoSyncEnabled && root.ready && !root.syncing)
                root.syncAll(true);
            else
                root.refresh();
        }
    ...
    function saveState(port) {
        writeProc.command = ["/bin/sh", "-c", "mkdir -p " + Lib.shellQuote(root.stateDir + "/omarchy-sfs") + " && printf '%s\\n' " + Lib.shellQuote(JSON.stringify({
                port: port,
                bin: root.resolvedBin,
                ts: Date.now()
    ...
    function locateIfIdle() {
        if (phase === "connected" || phase === "locating" || phase === "starting")
            return;
        locate();
    }
    ...
    BarIconButton {
        ...
        tooltipText: root.ready ? root.t("tooltipReady") : root.notFound ? root.t("tooltipMissing") : root.t("tooltipOff")
